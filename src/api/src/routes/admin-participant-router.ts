...
adminParticipantRouter.post("/", async (req: Request, res: Response) => {
  ...
  const existAddressQuery = db("PARTICIPANT_DATA")
    .withSchema(DB_SCHEMA)
    .innerJoin("PARTICIPANT", "PARTICIPANT_DATA.TOKEN", "PARTICIPANT.TOKEN")
  ...
  if (isNil(demographicGroup))
    existAddressQuery.whereNull("DEMOGRAPHIC_GROUP_ID");
  else existAddressQuery.where("DEMOGRAPHIC_GROUP_ID", demographicGroup);

  let existingAddresses = await existAddressQuery;
  ...
  if (fileData && fileData.headers && fileData.headers.length > 0) {
    ...
    for (let records of fileData.records) {
      ...
      for (let i = 0; i < fileData.headers.length; i++) {
        ...
        if (!demographics.includes(demo))
          return res.status(500).send(`Invalid demographic: ${demo}`);
      ...
      await db("PARTICIPANT").withSchema(DB_SCHEMA).insert({
        TOKEN: token,
        SID: survey,
        CREATE_DATE: new Date(),
        DEMOGRAPHIC_GROUP_ID: demographicGroup,
      });
    ...
    for (let address of addresses) {
      ...
      await db("PARTICIPANT_DATA")
        .withSchema(DB_SCHEMA)
        .insert({ TOKEN: token, EMAIL: address });
